(function(){
  ...
  */
  ...
  var cpfs = ['049-214 3421-1', '049-214 3421-1', '210.458.522-05', '735 500 794 - 22', '101.123-131x32'];
  var cpfsLimpos = cpfs.map(function(cpf) {
    var cpfClean = cleanCPF(cpf);
  ...
  */
  ...
  function formatCPF(cpf) {
    return cpf.replace(/(\d{3})(\d{3})(\d{3})(\d\d)/,'$1.$2.$3-$4');
  }
  cpfsLimpos.forEach( function(cpf) {
    console.log('CPF formatado',formatCPF(cpf));
  })
  ...
  */
  ...
  var aberturasTags = "<div><section><blockquote>Texto <img /></blockquote></section></div>";
  console.log(aberturasTags.match(/<[a-z]+>/g));
  ...
  */
  ...
  console.log(tagsVazias.match(/<[a-z]+><\/[a-z]+>/g));
  /*
  ...
  */
  ...
  var regex = /(<\w+>)([\wí\sáé]+)(<\/\w+>)/g;
  var textForInsert = 'O texto dentro da tag "[NOME DA TAG]" é "[TEXTO]"';
  var textForReplace = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>';

  function replaceText(regex, tag, text, closeTag){
    return tag + textForInsert.replace('[NOME DA TAG]', tag.replace(/[<>]/g,'')).replace('[TEXTO]', text) + closeTag + '\n';
  }

  console.log(textForReplace.replace(regex, replaceText));
})();
